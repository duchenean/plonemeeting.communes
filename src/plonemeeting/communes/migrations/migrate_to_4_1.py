...
from plonemeeting.communes.profiles.examples_fr.import_data import collegeMeeting
from plonemeeting.communes.profiles.examples_fr.import_data import data
...
class Migrate_To_4_1(PMMigrate_To_4_1):
    ...
    def _updateWFInterfaceNames(self):
        ...
        for cfg in self.tool.objectValues('MeetingConfig'):
            if cfg.getItemConditionsInterface() in (
                    'plonemeeting.communes.interfaces.IMeetingItemCollegeWorkflowConditions',
                    'plonemeeting.communes.interfaces.IMeetingItemCouncilWorkflowConditions'):
                cfg.setItemConditionsInterface(
                    'plonemeeting.communes.interfaces.IMeetingItemCommunesWorkflowConditions')
            if cfg.getItemActionsInterface() in (
                    'plonemeeting.communes.interfaces.IMeetingItemCollegeWorkflowActions',
                    'plonemeeting.communes.interfaces.IMeetingItemCouncilWorkflowActions'):
                cfg.setItemActionsInterface(
                    'plonemeeting.communes.interfaces.IMeetingItemCommunesWorkflowActions')
            if cfg.getMeetingConditionsInterface() in (
                    'plonemeeting.communes.interfaces.IMeetingCollegeWorkflowConditions',
                    'plonemeeting.communes.interfaces.IMeetingCouncilWorkflowConditions'):
                cfg.setMeetingConditionsInterface(
                    'plonemeeting.communes.interfaces.IMeetingCommunesWorkflowConditions')
            if cfg.getMeetingActionsInterface() in (
                    'plonemeeting.communes.interfaces.IMeetingCollegeWorkflowActions',
                    'plonemeeting.communes.interfaces.IMeetingCouncilWorkflowActions'):
                cfg.setMeetingActionsInterface(
                    'plonemeeting.communes.interfaces.IMeetingCommunesWorkflowActions')
        logger.info('Done.')
    ...
    def run(self,
            profile_name=u'profile-plonemeeting.communes:default',
            extra_omitted=[]):
...
def migrate(context):
    '''This migration function:
       ...
       1) Reinstall plonemeeting.communes and execute the plonemeeting.core migration;
